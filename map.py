...
class Map:
    ...
    def check_for_combat(self):
        ...
        for enemy in self.enemies:
            if pygame.math.Vector2(enemy.position).distance_to(self.player.position) < 50:
                self.in_combat = True
    ...
    def check_orb_collision(self):
        ...
        """
        if self.blue_orb and pygame.math.Vector2(self.orb_position).distance_to(self.player.position) < 25:
            self.game_over = True
    ...
    def handle_events(self):
        ...
        if keys[pygame.K_LEFT]:
            self.player.position[0] -= move_speed
        if keys[pygame.K_RIGHT]:
            self.player.position[0] += move_speed
        if keys[pygame.K_UP]:
            self.player.position[1] -= move_speed
    ...
    def draw(self):
        ...
        self.window.blit(self.player.image, (self.player.position[0], self.player.position[1]))
        for enemy in self.enemies:
            enemy.draw()
        if self.blue_orb:
    ...
    def load_player(self, character_type):
        self.player = Warrior("Jimothy", 100)
        # self.player = Character(character_type, armor, self.window)